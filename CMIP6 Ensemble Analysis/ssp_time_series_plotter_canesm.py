...
SSP Comparisons (Time series) - CanESM only
"""
...
ref_start, ref_end = 1995, 2015

# Plot time series of SAT anomalies by region for CanESM5 (Figure 0)
fig0, ax0 = plt.subplots(1, 5, figsize=(17,3))
ax0_ylim = [[2.6, 7.0, 3.5, 2.5, 2.1], [1.3, 4.5, 2.0, 1.3, 0.9]]
...
suptitle0 = 'SAT Anomalies of CanESM5 Ensemble'
...
for n in range(0,len(ax0_titles),1):
    ...
    if n==0:
        ax0[n].yaxis.set_minor_locator(MultipleLocator(0.5))
        ax0[n].yaxis.set_major_locator(MultipleLocator(1.0))       
...
ax1_ylim = [[7.0, 3.5, 2.5, 1.8], [4.5, 1.9, 1.0, 0.8]]
ax1_xlabels = ['Arctic SAT Anomaly (°C)', 'NH Mid-latitude SAT Anomaly (°C)', 
             'Tropical SAT Anomaly (°C)', 'SH extratropical SAT Anomaly (°C)']
colors1 = ['blue', 'darkorange', 'red', 'purple']
ax1_ylabel = 'Global SAT Anomaly (°C)'
suptitle1 = 'Regional SAT Anomalies vs Global Anomaly of CanESM5 Ensemble ('+str(project_start)+' - '+str(project_end)+')'
...
for n in range(0,len(ax1_xlabels),1):
    ...
    if project_end==2050:
        ...
        if n==0:
            ax1[n].yaxis.set_minor_locator(MultipleLocator(0.5))
...
suptitle2 = ' SAT Anomalies vs Global Anomaly of CanESM5 Ensemble ('+str(project_start)+' - '+str(project_end)+')'
...
for s in range(0, n_ssps, 1):
    curr_data_dir = data_dir + ssp_dirs[s] + canesm_dir
    ...
    for r in range(0, n_regions, 1): 
        ...
        # Plot the CanESM5 Regional vs Global temperature anomalies
        if r==1:
    ...
    # Add SSP text labels
    if project_end==2050:
        ax0[0].text(x=2000, y=2.2-s*0.2, s=ssp_labels[s], color=colors0[s], fontweight='bold')
        ax1[3].text(x=2.55, y=1.4-s*0.15, s=ssp_labels[s], color=colors0[s], fontsize='medium', fontweight='bold')
    else:
...
for r in range(0, n_regions-1, 1):
    avg_amp[r] = np.mean(amplifications[r,:])
    amp_std[r] = np.std(amplifications[r,:])
    amp_uncertainties[r] = (1/n_ssps)*np.sqrt(
          np.sum( np.square(amplifications_sigma[r,:]) )
        ) 
    print(r, 'mean amplification = {:.2f}'.format(avg_amp[r]), 
          '\nstddev = {:.2f}'.format(amp_std[r]), '\nuncertainty = {:.2f} \n'.format(amp_uncertainties[r]))

fig0.savefig('sat_anomalies_canesm_'+str(project_end)+'.png')
fig1.savefig('regional_sat_anomalies_vs_global_canesm_'+str(project_end)+'.png')
fig2.savefig('arctic_amplification_canesm_'+str(project_end)+'.png')
fig3.savefig('nh_amplification_canesm_'+str(project_end)+'.png')
fig4.savefig('tropical_amplification_canesm_'+str(project_end)+'.png')
fig5.savefig('sh_amplification_canesm_'+str(project_end)+'.png')    
