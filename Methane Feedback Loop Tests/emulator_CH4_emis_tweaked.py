...
A feedback loop of natural methane emissions has been added, with a corresponding
increase in natural emissions related to the Arctic temperature perturbation. The
feedback loop sensitivity is set by the parameter alpha.
"""
...
alpha = 0.50   
emis_ch4_nat_0 = 202.   # Prather et al. (2012)
emis_ch4_nat = 202.
emis_co_nat = 300.      # Olivie
emis_nox_nat = 15.      # Olivie
emis_voc_nat = 60.      # Olivie

# initial CH4 mixing ratio at the beginning of the time series (ppbf)

ch4_start_ppb = 731.41              # Meinshausen (2017) estimate for 1750

# diagnostic parameters

thorizon = [20, 100, 20, 100]                # time horizon for ARTP and AGTP diagnostic calculation (years)
year_ref = [2015, 2015, 2050, 2050]             # reference year for ARTP and AGTP diagnostic calculation (years)
...
cs_ch4_ar = np.array([0.31, 0.17, 0.16, 0.06])    # 60N-90N (Arctic) 
cs_ch4_ml = np.array([0.06, 0.24, 0.17, 0.07])    # 28N-60N (mid latitudes)
cs_ch4_tr = np.array([0.02, 0.10, 0.24, 0.09])    # 28N-28S (tropics)
cs_ch4_sh = np.array([0.  , 0.02, 0.05, 0.19])    # 28S-90S (southern hemisphere)
...
nstpyrt = nstpyr[nch4]
year_start_, year_end_ = 1750, 2050
n_steps = year_end_ - year_start_ + 1
scl0 = np.full((n_steps), 1.)
temp_ch4_ar = np.zeros((n_steps))
...
ecs0 = sum(tr_c) # sum of the temperature response coefficients
...
for n in range(nstp):
  ...
  # get T_arc for current year
  if False: #yearc>0:
    demis_i = (emis_yr[yearc-1] - emis_refr_yr[yearc-1]) * dtyrf 
    drf = rf_yr[yearc-1] - rf_refr_yr[yearc-2]
    ...
    frceff = (drf/dconc) * (1. + ch4_o3_scl + ch4_so4_scl + ch4_h2o_scl) 
    ...
    if scaling:
      scl0[yearc-1] = ecs0 / agtp_ch4_est
    
    for j in range(yearc+1,n_steps):                                                           # loop over years of temperature response to pulse emission
      timeyr_j = year_start[nch4] + j * dtyrf
      dt_emis = (j - yearc) * dtyrf
      for n in range(2):
        tr = tr + (tau_yr[yearc-1] * tr_c[n])/(tau_yr[yearc-1] - tr_d[n]) * ( np.exp(-dt_emis/tau_yr[yearc-1]) - np.exp(-dt_emis/tr_d[n]) )
      tr = tr * (ecs/ecs0)
      agtp_ch4 = frceff * tr

      artp_ch4_ar = (agtp_ch4/ecs0) * csh_ch4_ar * scl0[yearc-1]
      temp_ch4_ar[j] = temp_ch4_ar[j] + artp_ch4_ar * demis_i # Add contributions of yearc to all years j
    
    emis_ch4_all = emis_ch4_nat_0*(1 + alpha*relu(temp_ch4_ar[yearc-1] - T0))
  else:
        ...
        for kt in range(ntag_ch4[ne]):                                                       # loop over all sectors and regions
          if isec[ne][kt] is not None:
            ...
            for i in range(nstpyra):
              ...
              if ne == nch4:
                emissions_nat[i,ne] = emis_ch4_nat_0*(1 + alpha*relu(temp_ch4_ar[i] - T0))
