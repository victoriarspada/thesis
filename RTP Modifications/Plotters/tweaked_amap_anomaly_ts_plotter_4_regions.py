...
Same as amap_anomaly_ts_plotter_all_regions.py, but with the global anomaly
sublpot excluded
"""
import os
import numpy as np
...
ax1_xlabel = 'Year'
ax1_ylabel = ['Arctic Anomaly (°C)', 'NHML Anomaly (°C)', 'Tropical Anomaly (°C)', 
              'SH Anomaly (°C)']
minor_locs = [0.5, 0.25, 0.25, 0.25]
major_locs = [2.0, 1.0, 1.0, 1.0]

for i in range(0, 4, 1):
    ax1[i].set_xlabel(ax1_xlabel, fontsize='medium')
    ax1[i].set_ylabel(ax1_ylabel[i], fontsize='medium')
...
for i in range(0, len(ssp_labels), 1):
    ax1[0].text(x=2051, y=7.3-i*1.30, s=ssp_labels[i], color=colors0[i], 
