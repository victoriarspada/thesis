...
### input files
# emulator input 1: file for radiative forcing scensitivity from climate models
...
delta = D*area_frc_ar*cs_ar_old # PREV 0.5, 0.25
...
# regional temperature sensitivities for sulfate (AMAP, 2015)
cs[2][0] = list(np.array([0.31, 0.17, 0.16, 0.06])*(1+d1))    # 60N-90N (Arctic) 
cs[2][1] = list(np.array([0.06, 0.24, 0.17, 0.07])*(1+d2))    # 28N-60N (mid latitudes)
cs[2][2] = list(np.array([0.02, 0.10, 0.24, 0.09])*(1+d3))    # 28N-28S (tropics)
cs[2][3] = list(np.array([0.  , 0.02, 0.05, 0.19])*(1+d4))    # 28S-90S (southern hemisphere)
# # regional temperature sensitivities for organic aerosol (AMAP, 2015)
cs[3][0] = list(np.array([0.31, 0.17, 0.16, 0.06])*(1+d1))    # 60N-90N (Arctic) 
cs[3][1] = list(np.array([0.06, 0.24, 0.17, 0.07])*(1+d2))    # 28N-60N (mid latitudes)
cs[3][2] = list(np.array([0.02, 0.10, 0.24, 0.09])*(1+d3))    # 28N-28S (tropics)
cs[3][3] = list(np.array([0.  , 0.02, 0.05, 0.19])*(1+d4))    # 28S-90S (southern hemisphere)

# account for cs_ch4_sh[0] = 0 (get inf.)
d4 = -delta[0]/(2*area_frc_ml*cs_ml_old[0])
d5 = -delta[0]/(2*area_frc_tr*cs_tr_old[0])

cs[2][1][0] = cs_ml_old[0]*(1+d4)    # 28N-60N (mid latitudes)
cs[2][2][0] = cs_tr_old[0]*(1+d5)
cs[2][3][0] = 0
cs[3][1][0] = cs_ml_old[0]*(1+d4)   # 28N-60N (mid latitudes)
cs[3][2][0] = cs_tr_old[0]*(1+d5)
cs[3][3][0] = 0
...
outfile = './mod1_temp_aerosol_'+scenario+'_'+reference+'_'+model[case-1]+'_'+realm+'.nc'
