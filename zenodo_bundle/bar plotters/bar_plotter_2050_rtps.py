...
Plot results for various RTP adjustment settings
δ = 0.0, 0.10, 0.25, 0.50 (SSP5-8.5)
...
scenario = [ 'SSP126', 'SSP245', 'SSP370', 'SSP585',
            'CLE', 'MFR', 'CFM', 'MFR_SDS']
scenariox = [  '0.00', '0.10', '0.25', '0.50', 'CLE', 'MFR', 'CFM', 'MFR_SDS']
cmip6 = [ True, True, True, True, False, False, False, False, False, False ]
...
basedir = 'AMAP Projections/'
renormdirs = ['Renormed/', 'Renormed Mod3/', 'Renormed Mod2/', 'Renormed Mod1/']
ensembledirs = ['Renormed Ensemble/', 'Renormed Mod3 Ensemble/', 
                'Renormed Mod2 Ensemble/', 'Renormed Mod1 Ensemble/']
suffix = ['', '_all', '_all', '_all']
...
for cs in range(2): # For Arctic and global cases
  ...
  if case==0: # If Arctic scenario
    ...
    data_esm_min_ = np.genfromtxt(esmdat_min_arc, delimiter=",")[:,1]
    data_esm_max_ = np.genfromtxt(esmdat_max_arc, delimiter=",")[:,1]
    
    # Take mean or median
    data_esm_median_ = np.genfromtxt(esmdat_mean_arc, delimiter=",")[:,1]
    data_esm_median_ = np.genfromtxt(esmdat_median_arc, delimiter=",")[:,1]
    
    data_esm_min_arc = np.ones(5)*data_esm_min_[4]
    data_esm_max_arc = np.ones(5)*data_esm_max_[4]
    data_esm_median_arc = np.ones(5)*data_esm_median_[4]
    ...
    data_esm_min_ = np.genfromtxt(esmdat_min_glbl, delimiter=",")[:,1]
    data_esm_max_ = np.genfromtxt(esmdat_max_glbl, delimiter=",")[:,1]
    # Take mean or median
    data_esm_median_ = np.genfromtxt(esmdat_mean_glbl, delimiter=",")[:,1]
    data_esm_median_ = np.genfromtxt(esmdat_median_glbl, delimiter=",")[:,1]
    
    data_esm_min_glbl = np.ones(5)*data_esm_min_[4]
    data_esm_max_glbl = np.ones(5)*data_esm_max_[4]
    data_esm_median_glbl = np.ones(5)*data_esm_median_[4]

  for nz in range(nszen-1): # For each scenario
    # Simulated temperature change due to CO2 (take MEAN ECS output)
    if nz > 3:
      print('nz>3',scenario[nz])
      modconc = amap_data_dir+'temp_co2_'+scenario[nz]+'_zero_emulator_mean_'+refyrs+'.nc'
    else:
      print('nz<3',scenario[nz])
      modconc = basedir+renormdirs[nz]+'temp_co2_SSP585_zero_emulator_mean_'+refyrs+suffix[nz]+'.nc'
  ...
  if ch4_flag:
    ...
    for nz in range(nszen-1):
      if nz > 3:
        ...
        modconc = basedir+renormdirs[nz]+'temp_ch4_SSP585_zero_emulator_mean_'+refyrs+suffix[nz]+'.nc'
    ...
    if models[nm] == 'CanAM' or models[nm] == 'MRI-ESM' or models[nm] == 'emulator':
      ...
      for nz in range(nszen-1):
        if nz > 3:
          ...
          modconc = basedir+renormdirs[nz]+'temp_aerosol_SSP585_1750_'+models[nm]+'_mean_'+refyrs+suffix[nz]+'.nc'
  ...
  for nz in range(nszen-1):
    if case == 0:
      if nz > 3:
        ...
        modconc = basedir+ensembledirs[nz]+'dtemp_ARCTIC_SSP585.nc'
    else:
      if nz > 3:
        modconc = amap_data_dir_ensemble+'dtemp_GLOBAL_'+scenario[nz]+'.nc'
      else:
        modconc = basedir+ensembledirs[nz]+'dtemp_GLOBAL_SSP585.nc'
    ncfile = Dataset(modconc,"r",format="NETCDF4")
...
#  x = np.arange(nplt)  # the label locations
  ...
  fig.subplots_adjust(top=5.1,hspace=10.0, wspace=5.0)
  fig.suptitle('Projected SAT Anomalies for AMAP and CMIP6, SSP5-8.5 (2050)', 
               x=0.37)
  ...
  # automatically update ylim of axai when ylim of ax1 changes.
  ...
  axd.set_xlabel('δ', fontsize=ftsize_ax)
  axc.set_xlabel('δ', fontsize=ftsize_ax)
  ...
  for i2 in range(nsspy):
    if legend:
      ...
      rect = patches.Rectangle((xlegend,ylegend),xlegbox,ylegbox,facecolor=spltcolor[i2x,:],
                               edgecolor='black', linewidth=0)
      axl.add_patch(rect)
      xpos = xlegend + xlegbox + xoff
      ypos = ylegend + ylegbox/2 + yoff
      axl.text(xpos,ypos,spltlabel[i2x],verticalalignment='center',
               horizontalalignment='left',color='black',fontsize=ftsize_ax)
...
#    dbx = 0.1
  ...
  np.savetxt('2030_results_amap.txt', results, fmt='%s')
