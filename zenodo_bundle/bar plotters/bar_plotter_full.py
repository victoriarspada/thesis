...
Plot AMAP emulator results for various SSPs and AMAP scenarios
with bar plot showing contributions from each species and also the mean 
temperature anomaly in 2050 or 2030. This is shown for global and Arctic temperatures.
CMIP6 results (for comparison) are shown in black.

(autofits for 2030 and 2050)
"""
import matplotlib
...
for cs in range(2): # For Arctic and global cases
  ...
  if case==0: # If Arctic scenario
    ...
    data_esm_min_arc = np.genfromtxt(esmdat_min_arc, delimiter=",")[:,year_case]
    data_esm_max_arc = np.genfromtxt(esmdat_max_arc, delimiter=",")[:,year_case]
    # Take mean or median
    data_esm_median_arc = np.genfromtxt(esmdat_mean_arc, delimiter=",")[:,year_case]
    data_esm_median_arc = np.genfromtxt(esmdat_median_arc, delimiter=",")[:,year_case]
    ...
    data_esm_min_glbl = np.genfromtxt(esmdat_min_glbl, delimiter=",")[:,year_case]
    data_esm_max_glbl = np.genfromtxt(esmdat_max_glbl, delimiter=",")[:,year_case]
    # Take mean or median
    data_esm_median_glbl = np.genfromtxt(esmdat_mean_glbl, delimiter=",")[:,year_case]
    data_esm_median_glbl = np.genfromtxt(esmdat_median_glbl, delimiter=",")[:,year_case]
...
#  x = np.arange(nplt)  # the label locations
  ...
  fig.suptitle('Projected SAT Anomalies for AMAP and CMIP6 ({})'.format(year_end), x=0.32)
...
#    dbx = 0.1
  ...
  #plt.show()
  plt.savefig('amap_ensemble_vs_cmip6_2030.png', bbox_inches='tight', dpi=600)
