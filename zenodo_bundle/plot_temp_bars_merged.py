...
#scenariox = [ 'SSP1-2.6', 'SSP2-4.5', 'SSP3-7.0', 'SSP5-8.5' ]

#scenario = [ 'SSP126', 'SSP245', 'SSP370', 'SSP585', 'CLE', 'MFR', 'SLCF', 'SDS_MFR' ]
...
cmip6 = [ True, True, True, True, False, False, False, False ] # Do we have CMIP6 numbers?

#datadir = '/home/rks/python/AMAP/zenodo_aux/test/'
datadir = 'C:/Users/victo/Downloads/EngSci Year 4 Sem 1/ESC499 - Thesis/zenodo_bundle/AMAP Projections/'

#datadiroz = './'
datadiroz = 'C:/Users/victo/Downloads/EngSci Year 4 Sem 1/ESC499 - Thesis/zenodo_bundle/AMAP Projections Ozone/'

datadiresm = 'C:/Users/victo/Downloads/EngSci Year 4 Sem 1/ESC499 - Thesis/zenodo_bundle/FW__Decision,__Science_Advances_/'
...
year_start = 1990
year_end = 2050
#reltopd = True   # CMIP6 comparison
reltopd = False   # preindustrial reference
if reltopd:
  year_startx = 1986
  year_endx = 2005

fillv = -9999.
nszen = len(scenario) # Number of scenarios being plotted
nplt = nszen
nplt_cmip6 = 0
nplt_amap = 0
...
if nplt_amap != nplt_cmip6:
  print('nplt_amap != nplt_cmip6')
  # sys.exit()
print('nplt = '+str(nplt)+' '+str(nplt_amap))
...
# read ESM data sets
  if case==0:  # Start with AMAP numbers
    esmdat_median = datadiresm+'temp_AMAP_Arctic_median.csv'
    esmdat_min = datadiresm+'temp_AMAP_Arctic_low.csv'
    esmdat_max = datadiresm+'temp_AMAP_Arctic_high.csv'
    # mridat_median = datadiresm+'temp_AMAP_MRI_Arctic_median.csv'
    # mridat_min = datadiresm+'temp_AMAP_MRI_Arctic_low.csv'
    # mridat_max = datadiresm+'temp_AMAP_MRI_Arctic_high.csv'
  else:
    esmdat_median = datadiresm+'temp_AMAP_global_median.csv'
    esmdat_min = datadiresm+'temp_AMAP_global_low.csv'
    esmdat_max = datadiresm+'temp_AMAP_global_high.csv'
    # mridat_median = datadiresm+'temp_AMAP_MRI_global_median.csv'
    # mridat_min = datadiresm+'temp_AMAP_MRI_global_low.csv'
    # mridat_max = datadiresm+'temp_AMAP_MRI_global_high.csv'
  ...
  if case==0: #  CMIP6 Numbers
    esmdat_median = datadiresm+'temp_CMIP6_Arctic_median.csv'
    esmdat_min = datadiresm+'temp_CMIP6_Arctic_low.csv'
    esmdat_max = datadiresm+'temp_CMIP6_Arctic_high.csv'
  else:
    esmdat_median = datadiresm+'temp_CMIP6_global_median.csv'
    esmdat_min = datadiresm+'temp_CMIP6_global_low.csv'
    esmdat_max = datadiresm+'temp_CMIP6_global_high.csv'
  ...
  for nz in range(nszen):
    if not cmip6[nz] and scenario[nz] != 'MFR_SDS': # If CLE, MFR, or CFM
      if scenario[nz] != 'CFM' :
        temp_esm_median[nz,:] = data_esm_median[scenario[nz]]   # Index for the current scenario
    ...
    if cmip6[nz]: # If an SSP Scenario
      temp_esm_median[nz,:] = data_esm_cmip6_median[scenario[nz]] # Index for the current scenario
...
# simulated temperature change due to CO2
    ...
    modconc = datadir+'temp_co2_'+scenario[nz]+'_zero_mean_'+refyrs+'.nc'
...
# simulated temperature change due to CH4
  ...
  if ch4_flag:
    ...
    for nz in range(nszen):
      modconc = datadir+'temp_ch4_'+scenario[nz]+'_zero_mean_'+refyrs+'.nc'
      ncfile = Dataset(modconc,"r",format="NETCDF4")
...
# ozone
  ...
  for nz in range(nszen):
    ...
    # if not cmip6[nz]: # FOR AMAP DATASETS WE CAN INCLUDE OZONE EFFECTS
    #   modconc = datadiroz+'temp_ozone_'+scenario[nz]+'_1990_2050_1995_2014.nc'
    #   ncfile = Dataset(modconc,"r",format="NETCDF4")
...
# simulated temperature change due aerosol
...
#
  for nm in range(nmod):
    if models[nm] == 'CanAM' or models[nm] == 'MRI-ESM' or models[nm] == 'emulator':
      ...
      nreg1 = nreg
      for nz in range(nszen):
        modconc = datadir+'temp_aerosol_'+scenario[nz]+'_1750_'+models[nm]+'_mean_'+refyrs+'.nc'
#        modconc = datadir+'temp_aerosol_'+scenario[nz]+'_1750_'+models[nm]+'_mean_tst_'+refyrs+'.nc'
        ncfile = Dataset(modconc,"r",format="NETCDF4")
        nfsp     = get_coord_size('frcspec',ncfile)
        nssp     = get_coord_size('srcspec',ncfile)
...
# select time period for plotting and adjust model results to match observations in reference year
...
# uncertainty ranges
  ...
  for nz in range(nszen):
    if case == 0:
      modconc = datadir+'dtemp_ARCTIC_'+scenario[nz]+'.nc'
    else:
      modconc = datadir+'dtemp_GLOBAL_'+scenario[nz]+'.nc'
    ncfile = Dataset(modconc,"r",format="NETCDF4")
...
# print results
  ...
  for nz in range(nszen):
    print(scenario[nz])
    for nt in range(ntimem):
#    if temp_mod_time[nt] == 2030 or temp_mod_time[nt] == 2040 or temp_mod_time[nt] == 2050:
      if temp_mod_time[nt] == 2030 or temp_mod_time[nt] == 2050:
#    if temp_mod_time[nt] == 2050:
...
#      print(str(temp_mod_time[nt])+' '+str(temp_mod_plt[nz,nt])+' '+str(temp_mod_co2_plt[nz,nt]/temp_mod_spec_plt[nz,nt])+' '+str(temp_mod_ch4_plt[nz,nt]/temp_mod_spec_plt[nz,nt])+' '+str(temp_mod_s_plt[nz,nt]/temp_mod_spec_plt[nz,nt])+' '+str(temp_mod_bc_plt[nz,nt]/temp_mod_spec_plt[nz,nt])+' '+str(temp_mod_oc_plt[nz,nt]/temp_mod_spec_plt[nz,nt]))
...
#  fig, (axx,axy,axz) = plt.subplots(1, 3, gridspec_kw={'width_ratios': [3, 3, 1]})
...
#  axl = plt.subplot2grid((2,3),(1, 2))
  ...
  axa.set_ylabel(ylabel, fontsize=7)
  axc.set_ylabel(ylabel, fontsize=7)
  ...
  for i1 in range(2):
    for i2 in range(3):
      ...
      axb2.set_ylabel(ylabelx, color='grey', fontsize=7)  # we already handled the x-label with ax1
      axd2.set_ylabel(ylabelx, color='grey', fontsize=7)  # we already handled the x-label with ax1
...
#                  linestyle='--', zorder=0)
  ...
  for i2 in range(nsspy):
    if legend:
      ...
      axl.text(xpos,ypos,spltlabel[i2x],verticalalignment='center',horizontalalignment='left',color='black',fontsize=7)
...
#    dbx = 0.1
  ...
  if partial:
    if aoff:
      ...
      axa.set_ylabel(ylabel, fontsize=7)
      axa2.set_ylabel(ylabelx, color='grey', fontsize=7)  # we already handled the x-label with ax1
    if boff:
      ...
      axb.set_ylabel(ylabel, fontsize=7)
      axb2.set_ylabel(ylabelx, color='grey', fontsize=7)  # we already handled the x-label with ax1
    if coff:
      ...
      axc.set_ylabel(ylabel, fontsize=7)
      axc2.set_ylabel(ylabelx, color='grey', fontsize=7)  # we already handled the x-label with ax1
    if doff:
      ...
      axd.set_ylabel(ylabel, fontsize=7)
      axd2.set_ylabel(ylabelx, color='grey', fontsize=7)  # we already handled the x-label with ax1
    if loff:
