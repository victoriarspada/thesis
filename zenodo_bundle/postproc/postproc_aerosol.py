# python3 postprocessor for emulator-simulated temperature changes due to aerosol
...
modtemp = [ 'temp_aerosol_CLE_1750_emulator_mean' ]  #  input file names (without file extension)

year_start = 1995  # beginning of emulator reference time period
year_end = 2014  # end of emulator reference time period

# name of the directory that contains the input files with preliminary emulator-simulated temperatures
    
indir = '../'

# numerical parameters and routines for reading netcdf data
...
for nz in range(nszen):
  ...
  ds1.temp_ARCTIC.attrs = {('long_name', \
                            'Arctic temperature perturbation associated with regional aerosol emissions, relative to '+str(year_start)+'-'+str(year_end)), \
  ...
  ds2.temp_glb.attrs = {('long_name', \
                            'Global temperature perturbation associated with regional aerosol emissions, relative to '+str(year_start)+'-'+str(year_end)), \
                 ('standard_name', 'Arctic_temperature'), \
                 ('units', 'K'), \
                 ('_FillValue', fillv)}
  ds = xr.merge([ds1, ds2])
...
Licensee\'s use of the Data product.')}
  outfile = './'+modtemp[nz]+'_'+str(year_start)+'_'+str(year_end)+'.nc'
